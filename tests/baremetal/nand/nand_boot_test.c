/*
 * NAND boot path test for STMP3770 QEMU
 *
 * Builds an APBH DMA descriptor chain that mimics the ROM bootloader:
 *   READ_1ST (0x00) -> 5 address cycles -> READ_2ND (0x30) -> wait ready
 *   -> DMA read 4096 bytes from NAND page 0 to LOAD_ADDR -> jump to payload.
 */
...
void main(void) {
    ...
    /* Ensure descriptor writes are visible before kicking DMA. */
